...
class Show {
    ...
    private List<Seat> generateTieredSeats() {
        ...
        for (int i = 1; i <= 6; i++)
            newSeats.add(new Seat("A" + i, SeatTier.VIP));
        for (char row = 'B'; row <= 'C'; row++) {
            for (int i = 1; i <= 6; i++)
                newSeats.add(new Seat(row + String.valueOf(i), SeatTier.PREMIUM));
        }
        for (char row = 'D'; row <= 'E'; row++) {
            for (int i = 1; i <= 6; i++)
                newSeats.add(new Seat(row + String.valueOf(i), SeatTier.EXECUTIVE));
        }
        for (int i = 1; i <= 6; i++)
            newSeats.add(new Seat("F" + i, SeatTier.NORMAL));
        return newSeats;
    }
...
class Ticket {
    ...
    public String getEarnedPromoCode() {
    ...
    public void printTicket() {
        ...
        if (seatNumbers.length() > 36)
            seatNumbers = seatNumbers.substring(0, 33) + "...";
        System.out.printf("| Seats     : %-36s |\n", seatNumbers);

        if (!snacks.isEmpty()) {
            String snackList = snacks.stream().map(SnackItem::name).collect(Collectors.joining(", "));
            if (snackList.length() > 36)
                snackList = snackList.substring(0, 33) + "...";
...
public class MultiplexBookingSystem {
    ...
    private static void processBookingFlow() {
        ...
        Optional<Movie> selectedMovieOpt = movies.stream().filter(m -> m.getMovieId().equalsIgnoreCase(movieId))
                .findFirst();
        ...
        Optional<Show> selectedShowOpt = movieShows.stream().filter(s -> s.getShowId().equalsIgnoreCase(showId))
                .findFirst();
        ...
        List<Seat> seatsToBook = show.getSeats().stream().filter(seat -> requestedSeatIds.contains(seat.getSeatId()))
                .toList();
        ...
        if (!promo.isEmpty()) {
            if (usedPromoCodes.contains(promo)) {
                System.out.println(
                        "❌ ERROR: This promo code has already been claimed. Codes are strictly single-use per user.");
        ...
        Ticket ticket = new Ticket(show, seatsToBook, selectedSnacks, seatTotal, snackTotal, discount, gstAmount,
                convenienceFee, grandTotal, earnedPromoCode);
    ...
    private static void processCancellation() {
        ...
        Optional<Ticket> ticketOpt = userWallet.stream().filter(t -> t.getTicketId().equalsIgnoreCase(targetId))
                .findFirst();
        ...
        if (ticket.getEarnedPromoCode() != null) {
            ...
            System.out.println("⚠️ NOTICE: The bulk reward code (" + ticket.getEarnedPromoCode()
                    + ") generated by this booking has been revoked and destroyed.");
        }
    }
        ...
        for (Map.Entry<Movie, List<Show>> entry : showsByMovie.entrySet()) {
            ...
            for (Show show : movieShows) {
                ...
                System.out.printf("  [%s] %-10s | %s | Seats Sold: %02d/%d | Rev: Rs. %.2f\n",
                        show.getShowId(), show.getScreenName(), show.getTime(), soldSeats, show.getSeats().size(),
                        showRevenue);
            }
